...
Treat `cached_input_tokens` as a subset of `input_tokens`. The dashboard/API therefore expose:

- `input_tokens`: all input context processed by the model
- `cached_input_tokens`: input context served from cache
- `uncached_input_tokens`: `max(input_tokens - cached_input_tokens, 0)`
- `output_tokens`: tokens generated by the model

Track cached input with the same absolute high-water-mark strategy as the other token fields.
